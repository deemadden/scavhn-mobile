...
{
    Log4ObjC(@"Resolving request for definition for key: %@", key);

    TyphoonDefinition* cached = [self cachedDefinitionForKey:key me:me];
    if (!cached)
    {
        Log4ObjC(@"Definition for key: '%@' is not cached, building...", key);
        return [self buildDefinitionForKey:key me:me];
    }

    Log4ObjC(@"Using cached definition for key '%@.'", key);
    return cached;
}
...
+ (TyphoonDefinition*)buildDefinitionForKey:(NSString*)key me:(TyphoonAssembly*)me;
{
    ...
    Log4ObjC(@"Did finish building definition for key: '%@'", key);
...
+ (BOOL)dependencyForKey:(NSString*)key involvedInCircularDependencyInResolveStack:(NSArray*)resolveStack;
{
    if ([resolveStack count] >= 2)
    {
        ...
        {
            Log4ObjC(@"Circular dependency detected in definition for key '%@'. Breaking the cycle.", key);
            return YES;
        }
    }
...
- (void)dealloc
{
    Log4ObjC(@"$$$$$$ %@ in dealloc!", [self class]);
}
